...
// SetEscapeHTML enable or disables escape HTML chars '<' '>' '&' when marshaling.
// The default value is true.
func (kom *Map[K, V]) SetEscapeHTML(escape bool) {
	kom.escapeHTML = escape
}

func (kom *Map[K, V]) EscapeHTML() bool {
	return kom.escapeHTML
}

func (kom *Map[K, V]) init() {
	if kom.inner == nil {
...
// You should make sure 0 <= i < Len(OrderedMap), panic if out of bound.
...
func (kom *Map[K, V]) Pairs() *PairList[K, V] {
	...
	pairs.SetEscapeHTML(kom.escapeHTML)
